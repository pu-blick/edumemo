...
import { db, auth, collection, query, where, onSnapshot, addDoc, doc, updateDoc, deleteDoc, onAuthStateChanged } from '../lib/firebase';
...
const StudentDetail: React.FC = () => {
    ...
    const unsubAuth = onAuthStateChanged(auth, (user) => {
      ...
      onSnapshot(doc(db, 'students', studentId), (snap) => {
        if (snap.exists()) {
          ...
          setStudent(data); setEditName(data.name); setEditNumber(data.studentNumber);
        }
      });
      const q = query(collection(db, 'observations'), where('studentId', '==', studentId), where('ownerId', '==', user.uid));
      onSnapshot(q, (snap) => {
        const data = snap.docs.map(d => ({ id: d.id, ...d.data() } as Observation)).sort((a,b) => b.createdAt - a.createdAt);
        setObservations(data); setSelectedObsIds(new Set(data.map(o => o.id))); setIsLoading(false);
      });
    });
  }, [studentId]);
  ...
  const handleAddObservation = async (e?: React.FormEvent) => {
    e?.preventDefault(); const content = newObs.trim() || liveTranscription.trim();
    if (!content || !auth.currentUser || !student) return;
    await addDoc(collection(db, 'observations'), { studentId: student.id, classroomId: student.classroomId, content, normalizedContent: content.replace(/\s+/g, " "), ownerId: auth.currentUser.uid, createdAt: Date.now() });
    setNewObs(''); setLiveTranscription('');
  };
  ...
  const startVoiceRecording = async () => {
    try {
      ...
      const sessionPromise = ai.live.connect({
        ...
        config: { responseModalities: [Modality.AUDIO], inputAudioTranscription: {}, systemInstruction: '학생 관찰 내용을 받아적는 비서입니다.' },
        callbacks: {
          onopen: () => {
            setIsRecording(true);
            ...
            (window as any)._voiceSource = source; (window as any)._voiceProcessor = scriptProcessor;
          },
          onmessage: (m) => { if (m.serverContent?.inputTranscription) setLiveTranscription(p => p + m.serverContent!.inputTranscription!.text); },
  ...
  const stopVoiceRecording = () => {
    ...
    if (liveTranscription) { setNewObs(p => p + (p ? "\n" : "") + liveTranscription); setLiveTranscription(''); }
  };

  if (isLoading) return <div className="py-20 text-center font-bold">Loading...</div>;

  return (
    <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 animate-fade-in max-w-6xl mx-auto px-2">
      <div className="lg:col-span-8">
        <div className="mb-8 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          <Link to={`/classroom/${student?.classroomId}`} className="inline-flex items-center gap-1.5 text-indigo-600 font-bold hover:bg-indigo-50 px-3 py-1 rounded-lg text-[13px]"><ArrowLeft size={16}/> 목록</Link>
          ...
          <div className="flex items-center gap-4 text-right">
             {isEditingInfo ? (
               <div className="flex flex-col gap-2 items-end animate-fade-in">
                 ...
                 <div className="flex gap-2">
                   <button onClick={handleUpdateStudentInfo} className="px-3 py-1 bg-indigo-600 text-white rounded-lg font-bold text-[11px]">저장</button>
                   <button onClick={() => setIsEditingInfo(false)} className="px-3 py-1 bg-slate-100 text-slate-500 rounded-lg font-bold text-[11px]">취소</button>
                 </div>
               </div>
             ) : (
               <>
                 <div className="relative group">
                    <p className="text-[10px] text-slate-400 font-bold tracking-widest uppercase">{student?.studentNumber}</p>
        ...
        <div className="glass rounded-custom p-6 sm:p-8 mb-8 relative border border-white shadow-lg overflow-hidden">
          {isRecording && <div className="absolute inset-0 bg-indigo-600/5 backdrop-blur-md z-10 flex flex-col items-center justify-center animate-fade-in">
            <div className="flex gap-1.5 mb-6">{[1,2,3,4].map(i => <div key={i} className="w-2 h-10 bg-indigo-500 rounded-full animate-bounce" style={{animationDelay: `${i*0.1}s`}}/>)}</div>
          ...
          <textarea className="w-full h-32 p-5 rounded-xl bg-slate-50 border-none focus:bg-white focus:ring-4 focus:ring-indigo-500/5 font-medium text-base leading-relaxed transition-all placeholder:text-slate-300" placeholder="학생의 특징적인 모습을 기록해 주세요..." value={newObs} onChange={(e) => setNewObs(e.target.value)}/>
          <div className="mt-4 flex justify-end"><button onClick={handleAddObservation} className="bg-slate-800 text-white px-8 py-3.5 rounded-xl font-bold shadow-md hover:bg-black active:scale-95 transition-all text-sm">기록 저장</button></div>
        </div>
        ...
        <div className="space-y-4">
          <div className="flex justify-between items-center px-4">
            ...
            <button onClick={() => setSelectedObsIds(selectedObsIds.size === observations.length ? new Set() : new Set(observations.map(o => o.id)))} className="text-[11px] font-bold text-indigo-600 hover:underline">
              {selectedObsIds.size === observations.length ? '전체 해제' : '전체 선택'}
            </button>
          </div>
          {observations.map(obs => (
            <div key={obs.id} onClick={() => { const next = new Set(selectedObsIds); if(next.has(obs.id)) next.delete(obs.id); else next.add(obs.id); setSelectedObsIds(next); }} className={`glass-card rounded-xl p-6 flex gap-5 cursor-pointer border transition-all ${selectedObsIds.has(obs.id) ? 'border-indigo-200 bg-indigo-50/30' : 'border-transparent'}`}>
              <div className="shrink-0 mt-0.5">{selectedObsIds.has(obs.id) ? <CheckSquare className="text-indigo-600" size={20}/> : <Square className="text-slate-200" size={20}/>}</div>
              <div className="flex-1">
                <div className="flex justify-between items-start mb-2">
                  <span className="text-[10px] font-bold text-indigo-400 uppercase tracking-wider">{new Date(obs.createdAt).toLocaleString()}</span>
                  <button onClick={(e) => { e.stopPropagation(); if(confirm('기록을 삭제할까요?')) deleteDoc(doc(db, 'observations', obs.id)); }} className="text-slate-200 hover:text-rose-500 transition-all"><Trash2 size={16}/></button>
                </div>
      ...
      <div className="lg:col-span-4">
        <div className="glass rounded-custom p-6 sm:p-8 sticky top-24 border border-white shadow-xl bg-white/40">
          ...
          <div className="mb-8">
            <label className="block text-[10px] font-bold text-slate-400 uppercase tracking-widest mb-3 ml-1">추가 요청 사항 (선택)</label>
            <textarea 
              className="w-full h-24 p-4 rounded-xl border border-slate-100 bg-white font-medium text-sm focus:border-indigo-500 outline-none resize-none placeholder:text-slate-300" 
              placeholder="예: 긍정적인 성격 강조, 리더십 역량 포함 등..."
              value={extraContext}
              onChange={(e) => setExtraContext(e.target.value)}
            />
          </div>

          <button onClick={handleGenerateAI} disabled={isGenerating || selectedObsIds.size === 0} className="w-full bg-slate-800 text-white py-4 rounded-xl font-bold text-base flex items-center justify-center gap-2 hover:bg-black disabled:bg-slate-200 transition-all active:scale-95 mb-4">
          ...
          {draftResult && (
            <div className="mt-8 animate-slide-up">
              ...
              <div className="p-5 rounded-xl bg-white/90 border border-indigo-50 font-medium text-slate-700 text-sm leading-relaxed shadow-inner max-h-[300px] overflow-y-auto custom-scrollbar whitespace-pre-wrap">{draftResult}</div>
            </div>
          )}
        </div>
      </div>
